...
def notification_index(request):
    ...
    try:
        ...
        if caregiver_profile.role != 'CG':
            return JsonResponse(
                {'error': 'Only caregivers can view notifications.'},
                status=403
            )
    ...
    notifications = Notification.objects.filter(
        caregiver=caregiver_profile).order_by('-notified_at')
    ...
    Notification.objects.filter(
        caregiver=caregiver_profile, is_viewed=False
    ).update(is_viewed=True)

    # Fetch distinct end-users directly from the caregiver's profile
    assigned_endusers = caregiver_profile.end_users.values_list(
        'name', flat=True
    ).distinct()
...
def send_notification(request, icon_id):
    ...
    if not user_profile or user_profile.role != 'EU':
        return JsonResponse(
            {'error': 'Only end users can select icons.'},
            status=403
        )
    ...
    message = f'Notification sent to {caregiver.name} for icon: {icon.name}'
    return JsonResponse(
        {'message': message}
    )
...
def unread_notification_count(request):
    if (
        ...
        and request.user.userprofile.role == 'CG'
    ):
        unread_count = Notification.objects.filter(
            caregiver=request.user.userprofile, is_viewed=False
        ).count()
